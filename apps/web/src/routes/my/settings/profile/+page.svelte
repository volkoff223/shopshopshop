...
<div class="flex flex-col w-full h-full">
    <form action="?/updateProfile" method="POST" class="flex flex-col space-y-2 w-full" enctype="multipart/form-data" use:enhance={submitUpdateProfile}>
        ...
        <div class="form-control w-full max-w-lg">
            <lable for="avatar" class="lable font-medium pb-1">
                <span class="lable-text">Profile Picture</span>
            </lable>
            <label for="avatar" class="avatar w-32 rounded-full hover:cursor-pointer">
                ...
                <div class="w-32 rounded-full">
                    <img src={data.user?.avatar ? getImageURL(data.user?.collectionId, data.user?.id, data.user?.avatar) : defaultAvatar} alt="user avatar" id="avatar-preview" />
                </div>
            </label>
